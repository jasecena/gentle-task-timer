 ...
 *
 * Note there is deliberately no `expo-notifications` config plugin entry in
 * app.config.ts: it writes an `aps-environment` entitlement, which would need
 * the Push Notifications capability on the App ID. Local notifications need no
 * capability at all. The cost is that custom sound files cannot be bundled, so
 * alerts use the system default sound.
 */

/** Which feature an alert belongs to. Stored in the notification's `data`. */
export const ALERT_TAGS = { run: 'run', reminder: 'reminder' } as const;
...
/**
 * By default iOS suppresses a notification whose app is already in front. Both
 * features want the opposite: the alert is the point, foreground or not. The
 * list is left out so finished alerts do not silt up Notification Centre, and
 * the badge is left alone because a count of past alerts means nothing.
 */
...
/** Drops this tag's pending notifications, leaving the other feature's alone. */
